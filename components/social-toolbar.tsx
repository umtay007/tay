      ...
      {socials.map((social, index) => (
        <div key={social.name} className="relative" ref={openPopup === index ? popupRef : null}>
          ...
          {openPopup === index && social.accounts && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden min-w-[160px]">
              <div className="px-4 py-2.5 border-b border-white/10 text-center bg-white/5">
                <span className="text-xs font-semibold text-white/80 uppercase tracking-wider">{social.name}</span>
              </div>
              <div className="p-1.5">
                {social.accounts.map((account) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`flex items-center gap-2 px-4 py-2.5 text-sm text-white/70 hover:text-white hover:bg-white/10 rounded-xl transition-all duration-200 ${social.hoverColor}`}
